...
| #4-2 | Implementar design system básico do dashboard (cores, tipografia, componentes base). | - Identidade visual do projeto definida pela equipe;</br> - Projeto frontend configurado. | - Arquivo de estilos globais criado com variáveis de cor, fontes e componentes reutilizáveis prontos para uso em todo o dashboard. | 3 |
| #4-3 | Desenvolver tela inicial do dashboard com exibição do Score por região. | - Design system implementado;</br> - Score calculado e disponível via API. | - Página renderizando o índice de oportunidade de cada região de forma legível e com visual consistente. | 5 |