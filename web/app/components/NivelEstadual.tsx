...
import { BRASIL_PATHS, REGIOES_CONFIG, getViewBoxByUF } from "@/app/constants/map/brasilMapPaths"
...
export function NivelEstadual({ data = [], children, onUFChange }: NivelEstadualProps) {
  ...
  return (
    <section className="space-y-6 pt-4">
      ...
      <div className="flex flex-wrap gap-2">
        {ESTADOS_ORDENADOS.map((estado) => {
          ...
          return (
            <button
              key={estado.uf}
              onClick={() => handleUFChange(estado.uf)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 self-start card-base bg-white shadow-xl flex flex-col gap-3 p-4">
          <div className="flex items-center justify-between min-h-6">
            ...
            <span
              ...
              style={{ background: `${ufColor}20`, color: ufColor }}
            >
              {regiaoConfig.label}
            </span>
          </div>
          ...
          <svg viewBox={viewBox} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
            <defs>
              <filter id="neShadow">
                <feDropShadow dx="0" dy="3" stdDeviation="5" floodColor={ufColor} floodOpacity="0.35" />
              </filter>
            </defs>
            <path
              d={estadoAtivo.d}
          ...
          <div
            ...
            style={{ color: ufColor }}
          >
            <span>Cód. IBGE: {estadoAtivo.codIbge}</span>
            <span className="px-2 py-0.5 rounded-lg" style={{ background: `${ufColor}20` }}>
              {estadoAtivo.regiao}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {charts.slice(0, 2).map((title) => (
          <div key={title} className="card-base bg-white shadow-sm p-4 flex flex-col gap-3">
            ...
            <div className="flex-1 rounded-lg flex items-center justify-center min-h-27.5" />
          </div>
        ))}
      </div>
      <div className="card-base bg-white shadow-sm p-4 flex flex-col gap-3">
        <p className="text-[13px] font-bold text-gray-700 leading-snug">{charts[2]}</p>
        <div className="flex-1 rounded-lg flex items-center justify-center min-h-35" />
      </div>
    </>
  )
}
