...
import { AgingDividaChartProps } from "../types/components/AgingDividaChart"
...
export function AgingDividaChart({ ranking, agingRegiao, agingNacional, regiao }: AgingDividaChartProps) {
  ...
  const valores = useMemo(() => ranking.map(r => Number((r.aging * 100).toFixed(2))), [ranking])
  const mediaRegiao = Number((agingRegiao * 100).toFixed(2))
  const mediaNacional = Number((agingNacional * 100).toFixed(2))
  ...
  const options: ApexCharts.ApexOptions = useMemo(() => ({
    ...
    tooltip: {
      ...
      y: {
        ...
        title: { formatter: () => "Dívidas acima de 90d: " },
      },
    },
    legend: { show: false },
  }), [ufs, mediaRegiao, mediaNacional])

  const series = useMemo(() => [{ name: "Aging da Dívida", data: valores }], [valores])
