...
const EstadoPath_ = memo(function EstadoPath_({
  ...
  onClick,
}: EstadoProps & { isHidden?: boolean; onClick: () => void }) {
...
EstadoPath_.displayName = "EstadoPath_";
...
export function BrasilMap({ data = [], ufsOcultas }: BrasilMapProps) {
  ...
  const handleMapClick = useCallback((estado: EstadoPath) => {
    if (viewMode === "uf") {
      router.push(`/app/graficos/uf/${estado.uf.toLowerCase()}`);
    } else {
      const slug = estado.regiao.toLowerCase().replace(" ", "-");
  ...
  const getFill = useCallback((estado: EstadoPath): string => {
    if (heatmap) {
      if (viewMode === "uf") {
  ...
  const buildTooltipLines = useCallback((estado: EstadoPath): string[] => {
    const lines: string[] = [];
    if (viewMode === "uf") {
      const ufData = data.find((d) => d.uf === estado.uf);
    ...
    <div className="flex flex-col w-full h-fit min-h-150 relative overflow-hidden">
      ...
      <div className="w-full mt-6 py-4 flex flex-wrap gap-x-6 gap-y-2 text-[10px] sm:text-xs justify-center shrink-0 border-t border-gray-100">
        {heatmap ? (
          ...
          <p className="text-xs text-gray-400 italic">Cada estado exibe sua própria cor identificadora.</p>
        ) : (
