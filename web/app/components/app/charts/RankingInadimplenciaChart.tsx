...
import { getRankingInadimplenciaOptions } from "../../../services/ApexCharts/chartBuilder";

interface RankingInadimplenciaItem {
  uf: string
  taxa: number
}

interface RankingInadimplenciaChartProps {
...
export function RankingInadimplenciaChart({ ranking, taxaRegiao, taxaNacional, regiao }: RankingInadimplenciaChartProps) {

  // Lógica de formatação de dados isolada
  ...
  const options = useMemo(() =>
    getRankingInadimplenciaOptions(chartData.ufs, chartData.mediaRegiao, chartData.mediaNacional),
    [chartData]
  );

  const series = useMemo(() => [{ name: "Inadimplência Real", data: chartData.valores }], [chartData.valores]);
