...
export default function Dashboard() {
  ...
  const isMicro = viewMode === "uf";

  const categorias = isMicro ? UF_ORDER : ORDEM_REGIOES;
  const eixoISeries  = isMicro ? dadosEixoI_Ranking  : dadosEixoI_Score;
  const eixoIISeries = isMicro ? dadosEixoII_Ranking : dadosEixoII_Score;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4 px-3 sm:px-4 md:px-6 bg-[#F6F6F6]">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className={`grid grid-cols-1 ${isMicro ? "" : "md:grid-cols-2"} gap-6`}>
            ...
            />
            <RankingChart
              title="Inclusão e Expansão"
              info={`Composição do score de inclusão e expansão por ${isMicro ? "estado (UF)" : "macrorregião"}.`}
              series={eixoIISeries}
              categorias={categorias}
            />
          </div>
